...
with open(csv_file_path, "r") as csv_file:
    ...
    for row in reader:
        ...
        name_font = ImageFont.truetype("arial.ttf", size=20)
        title_font = ImageFont.truetype("arial.ttf", size=10)
        name = truncate_name(row["Name"], 29)
        title = row["Title"]
        draw = ImageDraw.Draw(template_image)
        draw.text((100, 170), name, font=name_font, fill=(0, 0, 0))
        draw.text((200, 143), title, font=title_font, fill=(0, 0, 0))
